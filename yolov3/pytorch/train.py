...
if __name__ == "__main__":
    ...
    # 数据预处理
    # exist_ok 递归创建时，若文件存在不报错
    os.makedirs(os.path.join(opt.model_output, "output"), exist_ok=True)
    os.makedirs(os.path.join(opt.model_output, "checkpoints"), exist_ok=True)

    # Get data configuration
    # parse_data_config，内含词典，keys: gpus, num_workers, classes, train, valid, names, backup, eval
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.checkpoint_interval == 0:
            torch.save(model.state_dict(), f"{opt.model_output}/checkpoints/yolov3_ckpt_%d.pth" % epoch)
