...
logger = logging.getLogger('demo.box_utils')
...
# ->
# 函数注释，告诉使用着参数类型是什么
# 框: feature map 上每个点6个框. [1, 1/2, 1/3, 2, 3, large box(size=sqrt(box_min, box_max))]
def generate_ssd_priors(specs: List[SSDSpec], image_size,
    ...
    for spec in specs:
        ...
        for j, i in itertools.product(range(spec.feature_map_size), repeat=2):
            # 使用scale 而不是直接使用feature map size!!!, scale会偏小一点，接近box的size
